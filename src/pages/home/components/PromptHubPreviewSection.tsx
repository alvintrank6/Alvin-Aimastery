...
export default function PromptHubPreviewSection() {
  ...
  const featuredPrompts = PROMPT_LIBRARY.slice(0, 3);
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-12">
          <div className="space-y-2">
            ...
            <h2 className="text-3xl md:text-5xl font-black text-[#1C2526] dark:text-white tracking-tight">
              Thư Viện <span className="text-[#9B2A4C] dark:text-cyan-400">Prompt Hub</span> Thực Chiến
            </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {featuredPrompts.map((item) => (
            <div
              key={item.id}
              className="p-6 rounded-3xl bg-white dark:bg-[#131B2E] border border-gray-200 dark:border-gray-800 shadow-xl flex flex-col justify-between space-y-4 hover:border-[#9B2A4C]/30 dark:hover:border-cyan-500/30 transition-all duration-300"
            >
              <div className="space-y-3">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-[#9B2A4C] dark:text-cyan-400 bg-[#9B2A4C]/10 dark:bg-cyan-400/10 px-3 py-1 rounded-full border border-[#9B2A4C]/20 dark:border-cyan-400/20">
                    {item.category}
                  </span>
                ...
                <div className="p-3 rounded-2xl bg-[#0E1524] text-cyan-300 font-mono text-[11px] space-y-1 overflow-hidden border border-gray-800">
              ...
              <div className="pt-4 border-t border-gray-100 dark:border-gray-800/80 flex items-center justify-between gap-2">
                <button
                  ...
                  className="flex-1 py-2.5 px-3 rounded-full border border-gray-300 dark:border-gray-700 hover:border-[#9B2A4C] dark:hover:border-cyan-400 text-xs font-bold text-[#1C2526] dark:text-white flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                >
                  {copiedId === item.id ? (
                    <>
                ...
                <button
                  ...
                  className="py-2.5 px-4 rounded-full bg-[#9B2A4C] text-white dark:bg-cyan-500 dark:text-black text-xs font-bold hover:scale-105 transition-transform flex items-center gap-1 cursor-pointer shadow-md"
                >
